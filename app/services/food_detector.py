...
class FoodDetector:
    ...
    def __init__(self):
        ...
        # Optional: limit concurrent inferences (helps in prod)
        if not hasattr(self, "_infer_semaphore"):
            self._infer_semaphore = asyncio.Semaphore(2)
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    MODEL_URL, timeout=60.0, follow_redirects=True
                )
            ...
            logger.info(
                f"Model downloaded successfully ({len(content) / 1024 / 1024:.1f} MB)"
            )
            return model_path
    ...
    async def _initialize_session(self):
        """Initialize the ONNX Runtime session (async + lock-protected)."""
    ...
    def _preprocess_image(self, image: Image.Image) -> np.ndarray:
        ...
        img_array = np.expand_dims(img_array, axis=0)  # -> NCHW (batch=1)
        return img_array

    def _load_and_preprocess_bytes(self, image_data: bytes) -> np.ndarray:
        """Load bytes into PIL and preprocess (runs in executor)."""
        image = Image.open(BytesIO(image_data))
        return self._preprocess_image(image)

    def _is_food_class(
        self, class_ids: List[int], probabilities: np.ndarray, threshold: float = 0.25
    ) -> Tuple[bool, float]:
        """Check if any of the top predicted classes are food-related."""
        food_score = 0.0

        for i, class_id in enumerate(class_ids[:5]):  # top 5
            if class_id in FOOD_CLASS_IDS:
                weight = 1.0 - (i * 0.15)
                food_score += float(probabilities[class_id]) * weight

        return food_score > threshold, food_score

    async def detect_food_in_image(
        self, image_input: Union[bytes, Image.Image], threshold: float = 0.25
    ) -> Tuple[bool, float]:
        """
        ...
        try:
            ...
            if isinstance(image_input, Image.Image):
                input_tensor = await loop.run_in_executor(
                    None, lambda: self._preprocess_image(image_input)
                )
            else:
                input_tensor = await loop.run_in_executor(
                    None, lambda: self._load_and_preprocess_bytes(image_input)
                )

            input_name = self._input_name or self._session.get_inputs()[0].name

            # Inference off the event loop (limit concurrency)
            async with self._infer_semaphore:
                outputs = await loop.run_in_executor(
                    None, lambda: self._session.run(None, {input_name: input_tensor})
                )
            ...
            is_food, score = self._is_food_class(
                top_5_indices.tolist(), probabilities, threshold
            )

            logger.debug(
                f"Food detection: is_food={is_food}, score={score:.3f} (threshold={threshold})"
            )
    ...
    def _calculate_image_hash_and_size(
    ...
    def _deduplicate_images(
        ...
        for url, score, img_hash, dimensions in image_results:
            ...
            for existing_hash, (existing_url, existing_score, existing_area) in list(
                unique_by_hash.items()
            ):
                try:
        ...
        all_unique = (
            [(url, score) for url, score, _ in unique_by_hash.values()] + no_hash_images
        )
    ...
    async def filter_food_images(
        ...
        logger.info(
            f"Food detector keeping {len(filtered_urls)}/{len(unique_images)} unique images after deduplication. "
            f"Raw unique: {unique_images}"
        )
        ...
        logger.info(
            f"Food detection complete: {valid_count}/{total_images} valid food images "
            f"({invalid_count} invalid) | total_time={total_time:.3f}s (dedup={dedup_time:.3f}s)"
        )
